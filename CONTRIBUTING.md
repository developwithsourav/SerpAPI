...
Comments from review bots work the same way. Read what they flag and comment in your own words on the ones worth fixing. Never click **Commit suggestion** on a bot's comment either: those commits also add a co-author line for the bot, and a bot is never credited here.
...
If the change was worth keeping, redo it as an ordinary commit ending `Step: #<number>`. Credit a teammate who suggested it with a `Co-authored-by:` line or by name in the commit body. Never credit a bot or an AI tool.
...
- Accept a bot's suggestion commit, or credit a bot or an AI tool as a co-author, contributor or reviewer
