...
Project skeleton (Divyansh). `pyproject.toml` with uv, Ruff and pytest, a committed `uv.lock`, the SerpApi client with its file cache and credit budget, and the Play Store adapter with tests as the reference every other adapter copies. No other adapter can start until this lands, and `lint-and-test` stays idle until `pyproject.toml` exists.

## Next

1. Repository settings (Sourav): turn off squash and rebase merging under Settings → General → Pull requests, turn off automatic code review, and mark `steps-guard` and `lint-and-test` as required checks. Both have run once now, so GitHub offers them in the picker. Finish the Ruleset list in `CONTRIBUTING.md`; "Restrict updates" and "Require linear history" stay off.
...
9. Submit a complete entry as soon as the app runs end to end. The website stamps the submission time at first submit and editing never resets it, so submitting early costs nothing.
